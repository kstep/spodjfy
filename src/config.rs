...
impl Config {
    ...
    pub fn config_file(&self) -> PathBuf { self.dirs.config_dir().join(SETTINGS_FILE) }

    pub fn spotify_token_file(&self) -> PathBuf { self.dirs.cache_dir().join(SPOTIFY_TOKEN_FILE) }
    ...
    pub fn save_settings(&self, settings: &Settings) -> Result<(), Error> {
        let data = toml::to_vec(settings).map_err(|error| std::io::Error::new(ErrorKind::InvalidData, error))?;
